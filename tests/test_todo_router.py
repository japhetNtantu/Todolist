...
def test_create_todos(client, cleanup, auth_headers):
    ...
    response = client.post("/todos", json=task_data, headers=auth_headers)
...
def test_update_todos(client, cleanup, auth_headers, task_id):
    ...
    response = client.put(f"/todos/{task_id}", json=modified_task_data, headers=auth_headers)
...
def test_update_todo_without_authentication(client, cleanup, task_id):
    ...
    response = client.put(f"/todos/{task_id}", json=modified_task_data)
